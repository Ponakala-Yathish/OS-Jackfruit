 ...
 *   - global list + mutex
 *   - periodic RSS check with soft/hard limit enforcement
 ...
 *
 * We use a mutex (not spinlock) because the timer callback runs
 * in softirq context on older kernels but process context on
 * newer ones. A mutex is safe from process context; the timer
 * on modern kernels (hrtimer/workqueue-backed) runs in process
 * context so mutex_lock is legal. This also avoids disabling
 * interrupts which would be required with spinlocks.
 * ============================================================== */
...
static void timer_callback(struct timer_list *t)
{
    ...
    /*
     * Use list_for_each_entry_safe so we can safely delete entries
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        /* Check hard limit first (takes priority) */
        ...
        /* Check soft limit — warn only once per entry */
